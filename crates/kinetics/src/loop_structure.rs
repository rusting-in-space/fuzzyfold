...
impl<'a, M: EnergyModel> LoopStructure<'a, M> {
    ...
    pub fn apply_del_move(&mut self, pl_idx: usize
        ...
        // NOTE: It may be prettier to accept i,j instead of pl_idx, 
        // but this one is just so much faster. So it is what it is!
        ...
        self.loop_neighbors.insert(lli, neighbors.clone());
...
mod tests {
    ...
    fn test_add_then_del_roundtrip() {
        ...
        for (i, j, de) in neighbors {
            ...
            let (pli, (p, q), rde) = ls.get_del_neighbors().first().cloned().unwrap();
